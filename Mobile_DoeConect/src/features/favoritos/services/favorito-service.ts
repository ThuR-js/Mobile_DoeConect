...
export const favoritoService = {
  // GET /api/v1/anuncio/favoritos/:usuarioId
  listar: (usuarioId: number) =>
    api.get<Favorito[]>(`/anuncio/favoritos/${usuarioId}`).then((r) => r.data),

  // POST /api/v1/anuncio/:id/favoritar?usuarioId=
  favoritar: (anuncioId: number, usuarioId: number) =>
    api
      .post<void>(`/anuncio/${anuncioId}/favoritar`, {}, { params: { usuarioId } })
      .then((r) => r.data),

  // DELETE /api/v1/anuncio/:id/favoritar?usuarioId=
  desfavoritar: (anuncioId: number, usuarioId: number) =>
    api
      .delete<void>(`/anuncio/${anuncioId}/favoritar`, { params: { usuarioId } })
      .then((r) => r.data),
};
